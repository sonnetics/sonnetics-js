...
describe("Detector.create", () => {
    it("throws when no model source provided", async () => {
        await expect(
            Detector.create(
                {} as { modelId?: string; path?: string; url?: string },
            ),
    ...
    it("throws when url is not http(s)", async () => {
        await expect(
            Detector.create({ url: "file:///tmp/model.tar.gz" }),
        ).rejects.toThrow("url must use http or https");
    });

    it("throws when url is invalid", async () => {
        await expect(Detector.create({ url: "not-a-url" })).rejects.toThrow(
            "url must be a valid HTTP(S) URL",
        );
    });

    it("throws when modelId is not a valid UUID", async () => {
        await expect(
